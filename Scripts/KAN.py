...
class KAN(nn.Module):
    ...
    def __init__(self, width=None, grid=3, k=3, noise_scale=0.1, noise_scale_base=0.1, base_fun=torch.nn.SiLU(), classification=False,
                symbolic_enabled=True, bias_trainable=True, grid_eps=1.0, grid_range=[-1, 1],hermite=False, sp_trainable=True, sb_trainable=True,
                device='cpu', seed=0):
        '''
        ...
        for l in range(self.depth):
            ...
            sp_batch = KANLayer(in_dim=width[l], out_dim=width[l + 1], num=grid, k=k, noise_scale=noise_scale, scale_base=scale_base, scale_sp=1., base_fun=base_fun, grid_eps=grid_eps, grid_range=grid_range, 
                                hermite=hermite,sp_trainable=sp_trainable,
    ...
    def apply_constraints_hermite(model,monotonic_vars):
        """
        Apply monotonic constraints to the Hermite spline basis functions and spline functions in the given model.

        Parameters:
        model (torch.nn.Module): The neural network model containing the activation functions and layers.
        monotonic_vars (dict): A dictionary where keys are variable indices and values are 1 for increasing monotonicity 
                            and -1 for decreasing monotonicity.

        Raises:
        ValueError: If there are no monotonic variables to impose monotonicity.

        Description:
        This function imposes monotonic constraints on the Hermite spline basis functions and spline functions in the model.
        It ensures that the weights associated with the basis functions and spline functions adhere to the specified 
        monotonic relationships. The constraints are applied as follows:
        
        1. For the first layer:
            - Extract the increasing and decreasing variables from the monotonic_vars dictionary.
            - Ensure that there are monotonic variables; otherwise, raise a ValueError.
            - Extract the weights associated with the basis function and clamp them to preserve monotonicity.
            - Impose monotonicity in the spline function by setting the weights to be non-negative.
            - Adjust the coefficients to ensure the monotonic relationship is maintained.

        2. For subsequent layers:
            - Impose increasing monotonicity of the basis function by setting the weights to be non-negative.
            - Impose increasing monotonicity of the spline function by setting the weights to be non-negative.
            - Adjust the coefficients to ensure the monotonic relationship is maintained.

        Note:
        - The function assumes that the grid is fixed and the same across layers.
        - The function modifies the model's layers in-place.
        """
        l=-1
        for layer in model.act_fun:
            ...
            if l == 0:
                with torch.no_grad():
                    # Extract the increasing and decreasing variables
                    increasing_vars = [k for k, v in monotonic_vars.items() if v == 1]
                    decreasing_vars = [k for k, v in monotonic_vars.items() if v == -1]

                    ## Check that both are not non-empty if not raise an error
                    if not increasing_vars and not decreasing_vars:
                        raise ValueError("There are no monotonic variables to impose monotonicity. Please check the monotonic_vars dictionary.")
                    
                    ## IMPOSE THE SPECIFIC MONOTONIC RELATIONSHIP IN THE BASIS FUNCTION
                    ## Extract the weights associated with the basis function
                    w_b = layer.scale_base.reshape(layer.out_dim, layer.in_dim)

                    if increasing_vars:
                        ## Clamp w_b to be greater than 0 -> Preserve increasing monotoniciy 
                        w_b[:,increasing_vars] = w_b[:,increasing_vars].clamp_min_(min=0)
                        layer.scale_base.data = w_b.reshape(layer.scale_base.data.shape)
                    if decreasing_vars:
                        ## Clamp w_b to be lower than 0 -> Preserve decreasing monotoniciy 
                        w_b[:,decreasing_vars] = w_b[:,decreasing_vars].clamp_max_(max=0)
                        layer.scale_base.data = w_b.reshape(layer.scale_base.data.shape)


                    ## IMPOSE THE MONOTONIC RELATIONSHIP IN THE SPLINE FUNCTION

                    # Impose monotonicity in the spline function by setting w_sp > 0 and adjusting the weights so the spline is increasing/decreasing
                    layer.scale_sp.data.clamp_(0.0)
                    
                    ## Extract the coefficients
                    # coef = layer.coef.data
                    coef = layer.coef[layer.weight_sharing].reshape(layer.out_dim, layer.in_dim,layer.coef.data.shape[1],2)
                    num_splines = layer.coef.data.shape[0]
                    num_grid_intervals = layer.coef.data.shape[1] - 1
                    
                    grid = model.act_fun[0].grid

                    for j in range(num_splines):
                        for i in range(num_grid_intervals):
                            for t in range(layer.out_dim):
                                if j in increasing_vars:
                                    ## Ensure p_i+1 >= p_i
                                    if coef[t,j,i, 0] > coef[t,j,i + 1, 0]:
                                        coef[t,j,i + 1, 0] = coef[t,j,i, 0]
                                    
                                    ## Compute delta_i as (p_i+1 - p_i)/(delta_i+1 - delta_i)
                                    delta = (coef[t,j,i + 1, 0] - coef[t,j,i, 0])/ (grid[0, i + 1] - grid[0, i])

                                    ## If delta_i = 0, set m_i = m_i+1 = 0
                                    if delta == 0:
                                        coef[t,j,i, 1] = 0
                                        coef[t,j,i + 1, 1] = 0
                                    else:
                                        ...
                                        ## Impose alpha_k^2 + beta_k^2 <= 9
                                        m0 = coef[t,j,i, 1]
                                        m1 = coef[t,j,i + 1, 1]
                                        alpha_k = m0 / delta
                                        beta_k = m1 / delta
                                        if alpha_k**2 + beta_k**2> 9:
                                            tau_k = 3 / (alpha_k**2 + beta_k**2)**0.5
                                            coef[t,j,i, 1] = tau_k * alpha_k * delta
                                            coef[t,j,i + 1, 1] = tau_k * beta_k * delta
                                elif j in decreasing_vars:
                                    ## Ensure p_i+1<=p_i
                                    if coef[t,j,i, 0] < coef[t,j,i + 1, 0]:
                                        coef[t,j,i + 1, 0] = coef[t,j,i, 0]
                                    ...
                                    if delta == 0:
                                        ...
                                        if alpha_k**2 + beta_k**2> 9:
                                            ...
                                            coef[t,j,i, 1] = tau_k * alpha_k * delta
                                            coef[t,j,i + 1, 1] = tau_k * beta_k * delta
                    layer.coef.data = coef.reshape(layer.coef.data.shape)
                
            else:
    
                with torch.no_grad():
                    
                    # Impose increasing monotonicity of the basis function by setting w_b > 0
                    layer.scale_base.data.clamp_(0.0)
                    # Impose increasing monotonicity of the spline function by setting w_sp > 0
                    layer.scale_sp.data.clamp_(0.0)
                    ...
                    for j in range(num_splines):
                        for i in range(num_grid_intervals):
                            ## Ensure p_i+1 >= p_i
                            if coef[j,i, 0] > coef[j,i + 1, 0]:
                                coef[j,i + 1, 0] = coef[j,i, 0]
                            
                            ## Compute delta_i as (p_i+1 - p_i)/(delta_i+1 - delta_i)
                            delta = (coef[j,i + 1, 0] - coef[j,i, 0])/ (grid[0, i + 1] - grid[0, i])
                            ...
                            if delta == 0:
                                ...
                                if alpha_k**2 + beta_k**2> 9:
                                    tau_k = 3 / (alpha_k**2 + beta_k**2)**0.5
                                    coef[j,i, 1] = tau_k * alpha_k * delta
                                    coef[j,i + 1, 1] = tau_k * beta_k * delta
                    layer.coef.data = coef

    def apply_constraints_hermite_vectorized(model, monotonic_vars):
        """
        Apply monotonic constraints to the Hermite spline basis functions and spline functions in the given model.
        ...
        for layer in model.act_fun:
            ...
            with torch.no_grad():
                if l == 0:
                    ...
                    if increasing_vars:
                        ## Ensure p_i+1 >= p_i
                        coef[:,increasing_vars,:,0] = torch.cummax(coef[:,increasing_vars,:,0],dim=2)[0]
                        
                        ## Compute delta_i as (p_i+1 - p_i)/(delta_i+1 - delta_i)
                        delta = (coef[:,increasing_vars, 1:, 0] - coef[:,increasing_vars, :-1, 0]) / (grid[0, 1:] - grid[0, :-1])

                        ## If delta_i = 0 set m_i = m_i+1 = 0
                        m0 = coef[:,increasing_vars, :-1, 1]
                        mask_delta_zero = delta == 0
                        coef[:,increasing_vars, :-1, 1] = torch.where(mask_delta_zero, torch.zeros_like(m0), m0)

                        m1 = coef[:,increasing_vars, 1:, 1]
                        mask_delta_zero = delta == 0
                        coef[:,increasing_vars, 1:, 1] = torch.where(mask_delta_zero, torch.zeros_like(m1), m1)

                        ## Convert delta_i = 0 to inf to avoid division by zero
                        delta[delta == 0] = float('inf')  # Avoid division by zero
                        mask_negative_m0 = coef[:,increasing_vars, :-1, 1] < 0
                        mask_negative_m1 = coef[:,increasing_vars, 1:, 1] < 0
                        coef[:,increasing_vars, :-1, 1] = torch.where(mask_negative_m0, torch.zeros_like(m0), coef[:,increasing_vars, :-1, 1])
                        coef[:,increasing_vars, 1:, 1] = torch.where(mask_negative_m1, torch.zeros_like(m1), coef[:,increasing_vars, 1:, 1])

                        alpha_k = coef[:,increasing_vars, :-1, 1] / delta
                        beta_k = coef[:,increasing_vars, 1:, 1] / delta
                        alpha_beta_norm = (alpha_k ** 2 + beta_k ** 2).sqrt()
                        
                        mask_exceed_norm = alpha_beta_norm > 3
                        tau_k = 3 / alpha_beta_norm
                        tau_k[~mask_exceed_norm] = 1

                        coef[:,increasing_vars, :-1, 1] = torch.where(mask_exceed_norm, tau_k * alpha_k * delta, coef[:,increasing_vars, :-1, 1])
                        coef[:,increasing_vars, 1:, 1] = torch.where(mask_exceed_norm, tau_k * beta_k * delta, coef[:,increasing_vars, 1:, 1])

                    if decreasing_vars:

                        ## Ensure p_i+1 <= p_i
                        coef[:,decreasing_vars,:,0] = torch.cummin(coef[:,decreasing_vars,:,0],dim=2)[0]
                        
                        ## Compute delta_i as (p_i+1 - p_i)/(delta_i+1 - delta_i)
                        delta = (coef[:,decreasing_vars, 1:, 0] - coef[:,decreasing_vars, :-1, 0]) / (grid[0, 1:] - grid[0, :-1])

                        ## If delta_i = 0 set m_i = m_i+1 = 0
                        m0 = coef[:,decreasing_vars, :-1, 1]
                        mask_delta_zero = delta == 0
                        coef[:,decreasing_vars, :-1, 1] = torch.where(mask_delta_zero, torch.zeros_like(m0), m0)

                        m1 = coef[:,decreasing_vars, 1:, 1]
                        mask_delta_zero = delta == 0
                        coef[:,decreasing_vars, 1:, 1] = torch.where(mask_delta_zero, torch.zeros_like(m1), m1)

                        ## Convert delta_i = 0 to inf to avoid division by zero
                        delta[delta == 0] = float('inf')  # Avoid division by zero
                        mask_negative_m0 = coef[:,decreasing_vars, :-1, 1] > 0
                        mask_negative_m1 = coef[:,decreasing_vars, 1:, 1] > 0
                        coef[:,decreasing_vars, :-1, 1] = torch.where(mask_negative_m0, torch.zeros_like(m0), coef[:,decreasing_vars, :-1, 1])
                        coef[:,decreasing_vars, 1:, 1] = torch.where(mask_negative_m1, torch.zeros_like(m1), coef[:,decreasing_vars, 1:, 1])

                        alpha_k = coef[:,decreasing_vars, :-1, 1] / delta
                        beta_k = coef[:,decreasing_vars, 1:, 1] / delta
                        alpha_beta_norm = (alpha_k ** 2 + beta_k ** 2).sqrt()
                        ...
                        coef[:,decreasing_vars, :-1, 1] = torch.where(mask_exceed_norm, tau_k * alpha_k * delta, coef[:,decreasing_vars, :-1, 1])
                        coef[:,decreasing_vars, 1:, 1] = torch.where(mask_exceed_norm, tau_k * beta_k * delta, coef[:,decreasing_vars, 1:, 1])
                    ...
                    ## Ensure p_i+1 >= p_i
                    coef[:,:,0] = torch.cummax(coef[:,:,0],dim=1)[0]
                    
                    ## Compute delta_i as (p_i+1 - p_i)/(delta_i+1 - delta_i)
                    delta = (coef[:, 1:, 0] - coef[:, :-1, 0]) / (grid[0, 1:] - grid[0, :-1])

                    ## If delta_i = 0 set m_i = m_i+1 = 0
                    m0 = coef[:, :-1, 1]
                    mask_delta_zero = delta == 0
                    coef[:, :-1, 1] = torch.where(mask_delta_zero, torch.zeros_like(m0), m0)

                    m1 = coef[:, 1:, 1]
                    mask_delta_zero = delta == 0
                    coef[:, 1:, 1] = torch.where(mask_delta_zero, torch.zeros_like(m1), m1)

                    ## Convert delta_i = 0 to inf to avoid division by zero
                    delta[delta == 0] = float('inf')  # Avoid division by zero
                    mask_negative_m0 = coef[:, :-1, 1] < 0
                    mask_negative_m1 = coef[:, 1:, 1] < 0
                    coef[:, :-1, 1] = torch.where(mask_negative_m0, torch.zeros_like(m0), coef[:, :-1, 1])
                    coef[:, 1:, 1] = torch.where(mask_negative_m1, torch.zeros_like(m1), coef[:, 1:, 1])

                    alpha_k = coef[:, :-1, 1] / delta
                    beta_k = coef[:, 1:, 1] / delta
                    alpha_beta_norm = (alpha_k ** 2 + beta_k ** 2).sqrt()
                    ...
                    coef[:, :-1, 1] = torch.where(mask_exceed_norm, tau_k * alpha_k * delta, coef[:, :-1, 1])
                    coef[:, 1:, 1] = torch.where(mask_exceed_norm, tau_k * beta_k * delta, coef[:, 1:, 1])
    ...
    def forward(self, x):
        ...
        for l in range(self.depth):
            ...
            ## If the layer is the last one, apply the activation function sigmoid if classification
            if l == self.depth-1 and self.classification:
                x = torch.sigmoid(x)
    ...
    def train(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0.,
               update_grid=True, grid_update_num=10, loss_fn=None, early_stopping_metric='loss', lr=1., stop_grid_update_step=50, batch=-1, monotonic = False, monotonic_vars = False,
               epsilon = 0.0, patience=100,small_mag_threshold=1e-16, small_reg_factor=1., metrics=None, sglr_avoid=False, save_fig=False,
        ...
        for _ in pbar:
            ...
            if opt == "LBFGS":
                ...
                if monotonic and self.hermite:
                    self.apply_constraints_hermite_vectorized(monotonic_vars=monotonic_vars)
    ...
    def plot_history(self, metrics=None):
        ...
        >>> f = lambda x: torch.exp(torch.sin(torch.pi*x[:,[0]]) + x[:,[1]]**2)
        >>> dataset = create_dataset(f, n_var=2)
        >>> results = model.train(dataset, opt='LBFGS', steps=50, lamb=0.01);
        >>> model.plot_history(results)
        '''
        ...
        if metrics != None:
            for i in range(len(metrics)):
                plt.plot(self.results[metrics[i].__name__], label=metrics[i].__name__)
        ## Scale logarithm
        plt.yscale('log')
        plt.legend()
        plt.show()
